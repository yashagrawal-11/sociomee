...
const C = {
  rose:"#ff3d8f", roseXlt:"#fff0f7", purple:"#7c3aed",
  purpleXlt:"#f5f3ff", ink:"#0d0015", slate:"#3b1f4e",
...
function AuthCallback() {
  ...
  useEffect(() => {
    ...
    if (token) {
      handleCallback(token)
        .then(() => { setStatus("success"); setTimeout(() => { window.location.href = "/"; }, 1200); })
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:"16px", background:`linear-gradient(135deg,${C.purpleXlt},${C.roseXlt})`, fontFamily:"sans-serif" }}>
      {status === "loading" && <><div style={{ width:"44px", height:"44px", borderRadius:"50%", border:`3px solid ${C.purple}22`, borderTopColor:C.purple, animation:"spin 0.7s linear infinite" }}/><p style={{ color:C.muted, fontSize:"14px", fontWeight:"600" }}>Signing you in…</p></>}
      {status === "success" && <><div style={{ fontSize:"48px" }}>✅</div><p style={{ color:C.success, fontSize:"16px", fontWeight:"700" }}>Logged in! Redirecting…</p></>}
      {status === "error" && <><div style={{ fontSize:"48px" }}>❌</div><p style={{ color:C.danger, fontSize:"14px", fontWeight:"600", maxWidth:"320px", textAlign:"center" }}>Login failed: {errMsg}</p><button onClick={() => { window.location.href = "/"; }} style={{ padding:"10px 24px", borderRadius:"99px", border:"none", background:`linear-gradient(135deg,${C.purple},${C.rose})`, color:C.white, fontWeight:"700", fontSize:"14px", cursor:"pointer" }}>Go back</button></>}
...
function LoginPage() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:`radial-gradient(ellipse at 20% 0%,${C.purpleXlt} 0%,#f8f4ff 35%,${C.roseXlt} 70%,#f0f9ff 100%)`, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans',sans-serif", padding:"32px 16px" }}>
      <div style={{ width:"100%", maxWidth:"420px" }}>
        <div style={{ textAlign:"center", marginBottom:"32px" }}>
          ...
          <h1 style={{ fontSize:"48px", fontWeight:"900", color:C.ink, lineHeight:1.0, letterSpacing:"-2px", marginBottom:"10px" }}>SocioMee<span style={{ color:C.rose }}>.</span></h1>
        ...
        <div style={{ background:C.glass, backdropFilter:"blur(24px)", border:`1px solid ${C.hairline}`, borderRadius:"24px", padding:"32px 28px", boxShadow:"0 20px 60px rgba(124,58,237,0.12)" }}>
          <h2 style={{ fontSize:"18px", fontWeight:"800", color:C.ink, marginBottom:"6px", textAlign:"center" }}>Sign in to get started</h2>
          <p style={{ fontSize:"13px", color:C.muted, textAlign:"center", marginBottom:"24px" }}>Free account · 30 credits/day · No credit card needed</p>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
            {[{icon:"📜",text:"Scripts up to 5000 words"},{icon:"🌐",text:"SEO packs — 8 platforms"},{icon:"🔬",text:"Deep research with GNews"},{icon:"🖼️",text:"Thumbnail analyzer"},{icon:"💳",text:"30 free credits/day"},{icon:"✦",text:"Upgrade to Pro anytime"}].map((f,i) => (
...
function LoadingScreen() {
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"16px", background:`linear-gradient(135deg,${C.purpleXlt},${C.roseXlt})`, fontFamily:"sans-serif" }}>
      ...
      <p style={{ color:C.muted, fontSize:"14px", fontWeight:"600", margin:0 }}>Loading SocioMee…</p>
