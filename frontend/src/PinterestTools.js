...
const C = {
  red:"rgba(255,255,255,0.8)", glass:"rgba(255,255,255,0.04)",
...
const FALLBACK_BOARDS = {
  ...
  travel:["Wanderlust Diaries ","Places to Go","Travel Goals Board","Dream Destinations","Adventure Awaits","Hidden Gems I Found","Travel Bucket List","World Explorer"],
  ...
  diy:["DIY I Must Try ","Craft Obsession","Make It Happen","Weekend Projects","DIY Saves","Handmade With Love","Creative Corner","Build & Create"],
...
function NichePills({ niches, active, onSelect, color="rgba(255,255,255,0.25)", activeBg="rgba(255,255,255,0.06)", activeText="#ffffff" }) {
  return (
          ...
          {descs.map((d,i)=>(
            <GlassCard key={i} style={{cursor:"pointer"}} onClick={()=>{navigator.clipboard.writeText(d);setCopied(i);setTimeout(()=>setCopied(null),2000);}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
                <div style={{width:"22px",height:"22px",borderRadius:"50%",background:"rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"11px",fontWeight:"800",color:"#ff6b8a"}}>{i+1}</div>
...
export function PinterestBoardNames() {
  ...
  return (
    <ToolCard title="Board Name Generator" icon="">
      <GlassCard style={{marginBottom:"16px"}}>
...
export function PinterestHashtags() {
  ...
  return (
    <ToolCard title="Hashtag Generator" icon="">
      <GlassCard style={{marginBottom:"16px"}}>
        <div style={{fontSize:"11px",fontWeight:"700",color:C.muted,textTransform:"uppercase",letterSpacing:"1px",marginBottom:"8px"}}>PIN TOPIC</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px",marginBottom:"16px"}}>
          ...
          <div>
            ...
            <input type="range" min="5" max="30" value={count} onChange={e=>setCount(Number(e.target.value))} style={{width:"100%",accentColor:"rgba(255,255,255,0.8)"}}/>
      ...
      {tags.length>0&&(
        <GlassCard>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
            ...
            <button onClick={copyAll} style={{padding:"6px 16px",borderRadius:"99px",border:"1px solid rgba(255,255,255,0.12)",background:"rgba(255,255,255,0.04)",color:copied?C.success:"#ff6b8a",fontSize:"11px",fontWeight:"700",cursor:"pointer",fontFamily:"inherit"}}>{copied?"✓ Copied All":"Copy All"}</button>
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:"6px"}}>
            {tags.map((tag,i)=><span key={i} onClick={()=>navigator.clipboard.writeText(tag)} style={{padding:"5px 12px",borderRadius:"99px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",color:"#ff6b8a",fontSize:"12px",fontWeight:"600",cursor:"pointer"}}>{tag}</span>)}
          </div>
        </GlassCard>
      )}
    </ToolCard>
