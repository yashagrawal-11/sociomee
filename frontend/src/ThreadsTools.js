...
function GenButton({ onClick, loading, label="Generate" }) {
  return (
    <button onClick={onClick} disabled={loading} style={{width:"100%",padding:"14px",borderRadius:"99px",border:"1.5px solid rgba(255,255,255,0.25)",background:loading?"rgba(255,255,255,0.03)":"rgba(255,255,255,0.08)",backdropFilter:"blur(16px)",color:"#fff",fontWeight:"800",fontSize:"14px",cursor:loading?"not-allowed":"pointer",fontFamily:"inherit",boxShadow:"none",transition:"all 0.3s",opacity:loading?0.6:1}}>
...
export function ThreadsGenerator() {
  ...
  const parts = thread ? [{label:"Thread 1 — Hook",text:thread.hook,emoji:"",accent:"rgba(255,255,255,0.7)"},{label:"Thread 2 — Body",text:thread.body,emoji:"📖",accent:"rgba(255,255,255,0.35)"},{label:"Thread 3 — CTA",text:thread.cta,emoji:"📢",accent:"#34d399"}] : [];
...
export function ThreadsBio() {
  ...
  return (
    <ToolCard title="Bio Writer" icon="">
      <GlassCard style={{marginBottom:"16px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
          {bios.map((bio,i)=>(
            <GlassCard key={i} style={{cursor:"pointer"}} onClick={()=>{navigator.clipboard.writeText(bio);setCopied(i);setTimeout(()=>setCopied(null),2000);}}>
              ...
              <div style={{marginTop:"8px",fontSize:"11px",color:bio.length>150?C.danger:C.muted}}>{bio.length}/150 chars {bio.length>150?"Too long":"✓ Good"}</div>
            </GlassCard>
          ))}
        </div>
      )}
