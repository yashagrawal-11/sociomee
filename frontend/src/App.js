            ...
            {(pack.description||pack.caption||pack.post)&&active!=="youtube"&&(
              <div>
                ...
                {editMode
                  ...
                  : <div className="dark-scroll" style={{ background:C.glass,borderRadius:"12px",padding:"14px 16px",fontSize:"13px",lineHeight:1.7,whiteSpace:"pre-wrap",color:C.ink,border:`1px solid ${C.hairline}`,maxHeight:"200px",overflowY:"auto" }}>{displayCap}</div>
                }
              </div>
            )}
    ...
    return (
      <div style={{ flex:1, border:`2px solid ${isSelected?"#a78bfa":isWinner&&mode==="ab"?"#22c55e":"rgba(255,255,255,0.08)"}`, borderRadius:"14px", overflow:"hidden", transition:"all 0.2s", position:"relative" }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.02)",padding:"12px",minHeight:"120px",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer" }}
          ...
          {preview
            ? <img src={preview} alt={`Thumbnail ${slot}`} style={{ width:"100%",aspectRatio:"16/9",objectFit:"contain",borderRadius:"6px",background:"#000" }}/>
...
const SEND_PLATFORMS = [
  { id:"youtube",   label:"YouTube",   color:"#ff0000", icon:"/icons/youtube.png", supports:["youtube"], requiresVideo:true },
  { id:"telegram",  label:"Telegram",  color:"#2aabee", icon:"/icons/telegram.png", supports:["telegram","all"] },
  { id:"pinterest", label:"Pinterest", color:"#e60023", icon:"/icons/pinterest.png", supports:["pinterest"] },
  { id:"discord",   label:"Discord",   color:"#5865f2", icon:"/icons/discord.png", svg:<svg width="14" height="14" viewBox="0 0 24 24" fill="#5865f2"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/></svg>, supports:["discord","all"] },
  { id:"linkedin",  label:"LinkedIn",  color:"#0a66c2", svg:<svg width="14" height="14" viewBox="0 0 24 24" fill="#0a66c2"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>, supports:["linkedin","all"], noVideo:true },
  { id:"facebook",  label:"Facebook (soon)",  color:"#1877f2", svg:<svg width="14" height="14" viewBox="0 0 24 24" fill="#1877f2"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>, supports:["facebook","all"], noVideo:true, comingSoon:true },
  { id:"instagram", label:"Instagram", color:"#e1306c", icon:"/icons/instagram.png", supports:["instagram","threads"] },
...
function PlatformSendBar({ result, platform, user, videoFile, selectedTitle, selectedHashtags=[] }) {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      conn.threads = false;
      try {
        const li = await fetch(`${BASE}/linkedin/status?user_id=${userId}`).then(r=>r.ok?r.json():null);
        conn.linkedin = li?.connected || false;
      } catch { conn.linkedin = false; }
      try {
        const fb = await fetch(`${BASE}/facebook/status?user_id=${userId}`).then(r=>r.ok?r.json():null);
        conn.facebook = fb?.connected || false;
      } catch { conn.facebook = false; }
      setConnections(conn);
    };
    check();
  }, [userId, result]);
  ...
  const isActive = (sp) => {
    ...
    if (sp.requiresVideo) return connections.youtube && !!videoFile;
    if (sp.noVideo && !!videoFile) return false;
  ...
  const handleSend = async (sp) => {
    ...
    // LinkedIn post
    if (sp.id === "linkedin") {
      setSendStatus(s=>({...s, linkedin:"sending"}));
      try {
        const token = localStorage.getItem("sociomee_token");
        const caption = result?.seo_packs?.linkedin?.caption || result?.caption || result?.post || "";
        const r = await fetch(`${BASE}/linkedin/post`, {
          method:"POST",
          headers:{"Content-Type":"application/json","Authorization":`Bearer ${token}`},
          body:JSON.stringify({user_id:userId, text:caption})
        });
        if(!r.ok) throw new Error("LinkedIn post failed");
        setSendStatus(s=>({...s, linkedin:"sent"}));
        setTimeout(()=>setSendStatus(s=>({...s, linkedin:"idle"})), 4000);
      } catch(e) {
        alert("LinkedIn post failed: " + e.message);
        setSendStatus(s=>({...s, linkedin:"idle"}));
      }
      return;
    }
    // Facebook post
    if (sp.id === "facebook") {
      setSendStatus(s=>({...s, facebook:"sending"}));
      try {
        const token = localStorage.getItem("sociomee_token");
        const caption = result?.seo_packs?.facebook?.caption || result?.caption || result?.post || "";
        const r = await fetch(`${BASE}/facebook/post`, {
          method:"POST",
          headers:{"Content-Type":"application/json","Authorization":`Bearer ${token}`},
          body:JSON.stringify({user_id:userId, text:caption})
        });
        if(!r.ok) throw new Error("Facebook post failed");
        setSendStatus(s=>({...s, facebook:"sent"}));
        setTimeout(()=>setSendStatus(s=>({...s, facebook:"idle"})), 4000);
      } catch(e) {
        alert("Facebook post failed: " + e.message);
        setSendStatus(s=>({...s, facebook:"idle"}));
      }
      return;
    }
    ...
    <div style={{ marginTop:"20px", paddingTop:"16px", borderTop:`1px solid ${C.hairline}` }}>
      {platform==="youtube" && (
        <div style={{ marginBottom:"12px",display:"flex",alignItems:"center",gap:"10px" }}>
          <span style={{ fontSize:"11px",fontWeight:"700",color:"rgba(255,255,255,0.4)",whiteSpace:"nowrap" }}>Visibility</span>
          <div style={{ width:"160px" }}>
            <CustomSelect value={ytPrivacy} onChange={setYtPrivacy} options={[{id:"public",label:"Public"},{id:"unlisted",label:"Unlisted"},{id:"private",label:"Private"}]} label="Public"/>
          </div>
        </div>
      )}
...
function ResultPanel({ result, platform, keyword, isPro, onUpgradeClick, user, onTitleSelect, videoFile, selectedTitle }) {
  ...
  const downloadPDF = () => {
    ...
    const variants = result.post_variants && result.post_variants.length > 0 ? result.post_variants : [result.caption || result.post || ""];
    const script = result.script_text || "";
    const desc = result.seo_description || result.youtube_description || "";
    const hashtags = (result.seo_hashtags || result.hashtags || []).join(" ");
    const hook = result.hook || "";

    const esc = (s) => (s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");

    const variantSections = variants.map((v, i) => `
      <div class="variant-header">Version ${i+1}</div>
      <div class="variant-box">${esc(v)}</div>
    `).join("");

    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"/><link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@700;900&family=Inter:wght@400;600;700;800;900&display=swap" rel="stylesheet"/><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',Arial,sans-serif;background:#0a0a0a;color:#ffffff;max-width:820px;margin:0 auto;padding:48px 40px;min-height:100vh}.header{padding:32px 36px;border-bottom:1px solid rgba(124,58,237,0.3);margin-bottom:40px;display:flex;align-items:center;justify-content:space-between}.logo{font-family:'Orbitron',sans-serif;font-size:26px;font-weight:900;color:#ffffff;letter-spacing:2px}.tagline{font-size:11px;color:rgba(255,255,255,0.4);letter-spacing:1px;margin-top:4px}.meta{font-size:11px;color:rgba(255,255,255,0.35);text-align:right;line-height:1.8}.section{margin-bottom:32px;padding-left:18px;border-left:3px solid #7c3aed}.section-title{font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#a78bfa;margin-bottom:12px}.title-box{background:rgba(124,58,237,0.1);border:1px solid rgba(124,58,237,0.2);border-radius:10px;padding:16px 20px;font-size:17px;font-weight:800;line-height:1.5;color:#fff}.hook-box{font-size:15px;font-weight:700;line-height:1.7;color:rgba(255,255,255,0.9);font-style:italic}.script{font-size:13px;line-height:2.1;white-space:pre-wrap;color:rgba(255,255,255,0.8)}.desc{font-size:12px;line-height:1.9;white-space:pre-wrap;color:rgba(255,255,255,0.7)}.hashtags{font-size:13px;color:#a78bfa;font-weight:600;line-height:2}.variant-header{font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#a78bfa;margin-bottom:10px;margin-top:20px}.variant-box{font-size:13px;line-height:2;white-space:pre-wrap;color:rgba(255,255,255,0.85);background:rgba(124,58,237,0.07);border:1px solid rgba(124,58,237,0.15);border-radius:10px;padding:16px 20px;margin-bottom:8px}.footer{margin-top:48px;text-align:center;font-size:11px;color:rgba(255,255,255,0.2);border-top:1px solid rgba(255,255,255,0.06);padding-top:20px;letter-spacing:1px}</style></head><body><div class="header"><div><div class="logo">SOCIOMEE</div><div class="tagline">ONE TOPIC. INFINITE CONTENT.</div></div><div class="meta">✦ AI Content Studio<br/>${plt}<br/>${date}</div></div>${title?`<div class="section"><div class="section-title">🎯 Topic</div><div class="title-box">${esc(title)}</div></div>`:""}${hook?`<div class="section"><div class="section-title">🎣 Hook</div><p class="hook-box">${esc(hook)}</p></div>`:""}${script?`<div class="section"><div class="section-title">📜 Full Script</div><div class="script">${esc(script)}</div></div>`:""}${variants.length>0?`<div class="section"><div class="section-title">📝 Content Versions</div>${variantSections}</div>`:""}${desc?`<div class="section"><div class="section-title">📋 Description</div><div class="desc">${esc(desc)}</div></div>`:""}${hashtags?`<div class="section"><div class="section-title">🏷️ Hashtags</div><div class="hashtags">${esc(hashtags)}</div></div>`:""}<div class="footer">GENERATED BY SOCIOMEE · SOCIOMEEAI.COM · AI CONTENT STUDIO</div></body></html>`;
...
function ChannelSettingsModal({ user, onClose, BASE, onUpgrade }) {
  const userId = localStorage.getItem("sociomee_user_id") || user?.user_id || "";
  const planStr = (user?.plan||user?.plan_label||"free").toLowerCase();
  const isPro = planStr !== "free";
  const isPlusPlan = planStr.includes("pro+") || planStr.includes("premium") || planStr.includes("plus");
  const maxAccounts = isPlusPlan ? 4 : isPro ? 2 : 0;
  ...
  const DisconnectBtn = ({onClick}) => <button onClick={onClick} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.12)"} onMouseLeave={e=>e.currentTarget.style.background="rgba(255,255,255,0.06)"} style={{padding:"4px 10px",borderRadius:"99px",border:"1px solid rgba(255,255,255,0.15)",background:"rgba(255,255,255,0.06)",backdropFilter:"blur(8px)",color:"rgba(255,255,255,0.75)",fontSize:"10px",fontWeight:"600",cursor:"pointer",fontFamily:"inherit",flexShrink:0,transition:"background 0.15s"}}>Disconnect</button>;
  ...
  const Sec = ({icon,label,count,children}) => (
    <div style={{marginBottom:"20px"}}>
      <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"10px"}}>
        ...
        {isPro && <span style={{fontSize:"10px",fontWeight:"700",color:"rgba(167,139,250,0.8)",marginLeft:"auto"}}>{count}/{maxAccounts}</span>}
      </div>
      {children}
    </div>
  );
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",backdropFilter:"blur(10px)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:"460px",background:"rgba(10,10,10,0.98)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"20px",padding:"24px",maxHeight:"82vh",overflowY:"auto",scrollbarWidth:"none",msOverflowStyle:"none",boxShadow:"0 24px 80px rgba(0,0,0,0.9),0 0 0px rgba(0,0,0,0)"}}>
        ...
        {loading ? <div style={{textAlign:"center",padding:"40px",color:"rgba(255,255,255,0.3)",fontSize:"13px"}}>Loading...</div> : (<>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"12px"}}><div style={{display:"flex",alignItems:"center",gap:"8px"}}><img src="/icons/pinterest.png" style={{width:16,height:16,objectFit:"contain"}} alt="pt"/><span style={{fontSize:"10px",fontWeight:"800",letterSpacing:"1.5px",textTransform:"uppercase",color:"rgba(255,255,255,0.4)"}}>Pinterest</span></div>{isPro && <span style={{fontSize:"11px",fontWeight:"700",color:"rgba(167,139,250,0.8)"}}>{pinterestStatus?.connected?1:0}/{maxAccounts}</span>}</div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",margin:"0 0 10px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"8px"}}><img src="/icons/threads.png" style={{width:16,height:16,objectFit:"contain"}} alt=""/><span style={{fontSize:"10px",fontWeight:"800",letterSpacing:"1.5px",textTransform:"uppercase",color:"rgba(255,255,255,0.3)"}}>Threads</span></div>
            {isPro && <span style={{fontSize:"10px",fontWeight:"700",color:"rgba(167,139,250,0.8)"}}>{threadsStatus?.connected?1:0}/{maxAccounts}</span>}
          </div>
          ...
          <div style={{height:"1px",background:"rgba(255,255,255,0.07)",margin:"16px 0"}}/>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",margin:"0 0 10px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"8px"}}><img src="/icons/instagram.png" style={{width:16,height:16,objectFit:"contain"}} alt=""/><span style={{fontSize:"10px",fontWeight:"800",letterSpacing:"1.5px",textTransform:"uppercase",color:"rgba(255,255,255,0.3)"}}>Instagram</span></div>
            {isPro && <span style={{fontSize:"10px",fontWeight:"700",color:"rgba(167,139,250,0.8)"}}>{igStatus?.connected?1:0}/{maxAccounts}</span>}
          </div>
          {igStatus?.connected ? (
            <div style={{display:"flex",alignItems:"center",gap:"12px",padding:"12px 14px",borderRadius:"12px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",marginBottom:"8px"}}>
          ...
          <div style={{height:"1px",background:"rgba(255,255,255,0.07)",margin:"16px 0"}}/>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",margin:"0 0 10px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"8px"}}><img src="/icons/facebook.png" style={{width:16,height:16,objectFit:"contain"}} alt=""/><span style={{fontSize:"10px",fontWeight:"800",letterSpacing:"1.5px",textTransform:"uppercase",color:"rgba(255,255,255,0.3)"}}>Facebook</span></div>
            <span style={{fontSize:"10px",color:"rgba(255,255,255,0.2)",fontWeight:"600"}}>Coming Soon</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:"12px",padding:"12px 14px",borderRadius:"12px",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",marginBottom:"8px"}}>
            <img src="/icons/facebook.png" style={{width:24,height:24,objectFit:"contain",flexShrink:0}} alt="Facebook"/>
            <span style={{fontSize:"13px",fontWeight:"600",color:"rgba(255,255,255,0.4)",flex:1}}>Facebook pages integration</span>
          </div>
        </>)}
      </div>
    </div>
...
function SkeletonScreen() {
  return (
    <div style={{minHeight:"100vh",background:"#0a0a0a",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{width:"36px",height:"36px",borderRadius:"50%",border:"2px solid rgba(255,255,255,0.06)",borderTopColor:"rgba(255,255,255,0.3)",animation:"sk-spin 0.8s linear infinite"}}/>
...
export default function App() {
          ...
          {openGroups.instagram && (
            <div style={{paddingLeft:"10px",borderLeft:"2px solid rgba(225,48,108,0.2)",marginLeft:"14px",display:"flex",flexDirection:"column",gap:"1px",marginBottom:"4px"}}>
              {[
                ...
                {tab:"qrgenerator",label:"QR Generator",icon:<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><path d="M14 14h3v3m0 4h4m-4 0v-4m-3 4h-1v-3"/></svg>},
                {tab:"linkinbio",label:"Link in Bio",icon:<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>},
                {tab:"hookgenerator",label:t("hookGenerator"),icon:<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 6h16M4 12h8m-8 6h16"/></svg>},
                {tab:"translator",label:t("translator"),icon:<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>},
                {tab:"texttaudio",label:t("textToAudio"),icon:<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>},
              ].map(item=>(
  ...
  style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"13px",borderRadius:"99px",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.18)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",color:"#fff",fontWeight:"800",fontSize:"14px",cursor:"pointer",fontFamily:"inherit",boxShadow:"inset 0 1px 0 rgba(255,255,255,0.15),0 4px 16px rgba(0,0,0,0.3)",transition:"all 0.35s cubic-bezier(0.34,1.56,0.64,1)"}}>✦ Upgrade to Pro ₹499/mo</button>
      ...
      {activeTab==="hashtags"   && isLoggedIn && <div style={{background:"rgba(255,255,255,0.04)",border:"1.5px solid rgba(255,255,255,0.08)",borderRadius:"18px",padding:"24px"}}><HashtagGenerator user={user}/></div>}
          ...
          {activeTab==="hookgenerator"&&isLoggedIn&& <HookGenerator user={user}/>}
          ...
          {activeTab==="linkinbio" && isLoggedIn && <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"60vh"}}><div style={{background:"rgba(255,255,255,0.04)",border:"1.5px solid rgba(255,255,255,0.08)",borderRadius:"18px",padding:"32px",width:"100%",maxWidth:"560px"}}><PlanGate plan={user?.plan||"free"} required="pro" toolName="Link in Bio" onUpgrade={()=>window.location.href="/pricing?from=app"}><LinkInBio user={user}/></PlanGate></div></div>}
          {activeTab==="qrgenerator" && isLoggedIn && <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"60vh"}}><div style={{background:"rgba(255,255,255,0.04)",border:"1.5px solid rgba(255,255,255,0.08)",borderRadius:"18px",padding:"32px",width:"100%",maxWidth:"560px"}}><PlanGate plan={user?.plan||"free"} required="pro" toolName="QR Code Generator" onUpgrade={()=>window.location.href="/pricing?from=app"}><QRGenerator user={user}/></PlanGate></div></div>}
                ...
                <div>
                  ...
                  {["Content quality","More platform support","Faster generation","Better UI and design","Lower pricing","More store tools"].map(opt=>(
                    <button key={opt} onClick={()=>setFeedbackAnswers(p=>({...p,improve:opt}))} style={{display:"flex",alignItems:"center",gap:"10px",width:"100%",padding:"10px 14px",borderRadius:"10px",border:`1px solid ${feedbackAnswers.improve===opt?"rgba(255,255,255,0.3)":"rgba(255,255,255,0.06)"}`,background:feedbackAnswers.improve===opt?"rgba(255,255,255,0.08)":"rgba(255,255,255,0.02)",color:"rgba(255,255,255,0.8)",fontSize:"13px",cursor:"pointer",fontFamily:"inherit",textAlign:"left",marginBottom:"6px",transition:"all 0.15s"}}>
                      <div style={{width:"18px",height:"18px",borderRadius:"5px",border:`1px solid ${feedbackAnswers.improve===opt?"#fff":"rgba(255,255,255,0.2)"}`,background:feedbackAnswers.improve===opt?"rgba(255,255,255,0.15)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.15s"}}>{feedbackAnswers.improve===opt && <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}</div>
                      {opt}
                    </button>
                  ))}
