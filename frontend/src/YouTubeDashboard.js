 ...
 * Shows channel stats, views/subs graph, top videos, AI growth prediction,
 * and Auto-Upload with AI SEO (Pro/Premium feature).
 */
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"52px 24px", gap:"20px" }}>
      ...
      <div style={{ textAlign:"center" }}>
        ...
        <p style={{ fontSize:"13px", color:C.muted, lineHeight:1.6, maxWidth:"340px" }}>
          Get real analytics, AI-powered growth predictions, and channel insights.
        </p>
      </div>
      <div style={{ display:"flex", flexDirection:"column", gap:"8px", width:"100%", maxWidth:"320px" }}>
        {["📊 Views & subscribers graph", "🎬 Top performing videos", "🤖 AI growth prediction", "📤 Auto-upload with AI SEO (Pro/Premium)"].map((f, i) => (
      ...
      <p style={{ fontSize:"11px", color:C.muted }}>Secured by Google OAuth 2.0</p>
    </div>
  );
}
...
export default function YouTubeDashboard({ user, topic = "" }) {
  ...
  if (!connected) return <ConnectYouTube userId={userId} onConnected={() => { setConnected(true); load(); }} />;
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ display:"flex", gap:"8px", marginBottom:"20px" }}>
        {[["analytics","📊 Analytics"],["upload","📤 Auto-Upload"]].map(([key, label]) => (
          <button key={key} onClick={() => setActiveTab(key)} style={{ padding:"9px 18px", borderRadius:"12px", border:`1.5px solid ${activeTab===key ? C.yt : C.hairline}`, background:activeTab===key ? `${C.yt}15` : C.glass, color:activeTab===key ? C.yt : C.muted, fontWeight:"800", fontSize:"13px", cursor:"pointer", fontFamily:"inherit" }}>
            {label}
          </button>
        ))}
      </div>
