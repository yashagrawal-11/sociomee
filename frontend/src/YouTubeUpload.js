 ...
 * SEO generation temporarily removed — upload first, SEO coming back soon.
 */
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 24px", gap: "16px", textAlign: "center" }}>
      ...
      <p style={{ fontSize: "13px", color: C.muted, lineHeight: 1.6, maxWidth: "320px" }}>
        Upload videos to YouTube automatically. Just drop your video and we handle everything.
      </p>
...
export default function YouTubeUpload({ user }) {
  ...
  const handleUpload = async () => {
    ...
    if (!topic.trim())   { setError("Enter a video title/topic"); return; }
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", paddingBottom: "20px" }}>
      ...
      {/* Success */}
      {result && (
        <div style={{ background: `${C.success}12`, border: `1.5px solid ${C.success}44`, borderRadius: "16px", padding: "18px", marginBottom: "20px" }}>
          <div style={{ fontSize: "13px", fontWeight: "900", color: C.success, marginBottom: "10px" }}>✅ Video Uploaded Successfully!</div>
          <div style={{ fontSize: "13px", color: C.ink, fontWeight: "700", marginBottom: "6px" }}>{result.title}</div>
          {result.scheduled
            ? <div style={{ fontSize: "12px", color: C.warn }}>⏰ Scheduled: {result.best_time?.ist_label || new Date(result.scheduled).toLocaleString("en-IN")}</div>
      ...
      >
        ...
        {videoPreview ? (
          ...
          <>
            ...
            <div style={{ fontSize: "12px", color: C.muted, marginTop: "4px" }}>Supports MP4, MOV, AVI · Max 256 MB · Shorts & Long videos</div>
          </>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "14px" }}>
        {[["video", "🎬 Long Video"], ["short", "⚡ Short (< 60s)"]].map(([val, label]) => (
          <button key={val} onClick={() => setVideoType(val)} style={{ flex: 1, padding: "10px", borderRadius: "12px", border: `1.5px solid ${videoType === val ? C.yt : C.hairline}`, background: videoType === val ? `${C.yt}15` : C.glass, color: videoType === val ? C.yt : C.muted, fontWeight: "700", fontSize: "13px", cursor: "pointer", fontFamily: "inherit" }}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{ marginBottom: "14px" }}>
        ...
        <input
          ...
          style={{ width: "100%", padding: "11px 14px", borderRadius: "12px", border: `1.5px solid ${C.hairline}`, background: C.glass, color: C.ink, fontSize: "13px", fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}
        />
      </div>
      ...
      <div style={{ marginBottom: "14px" }}>
        ...
        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
          {[
            ...
            <button key={val} onClick={() => setScheduleType(val)} style={{ padding: "8px 14px", borderRadius: "10px", border: `1.5px solid ${scheduleType === val ? C.purple : C.hairline}`, background: scheduleType === val ? `${C.purple}18` : C.glass, color: scheduleType === val ? C.purple : C.muted, fontWeight: "700", fontSize: "12px", cursor: "pointer", fontFamily: "inherit" }}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", gap: "6px" }}>
          {[["public", "🌍 Public"], ["unlisted", "🔗 Unlisted"], ["private", "🔒 Private"]].map(([val, label]) => (
            <button key={val} onClick={() => setPrivacy(val)} style={{ flex: 1, padding: "8px", borderRadius: "10px", border: `1.5px solid ${privacy === val ? C.teal : C.hairline}`, background: privacy === val ? `${C.teal}18` : C.glass, color: privacy === val ? C.teal : C.muted, fontWeight: "700", fontSize: "11.5px", cursor: "pointer", fontFamily: "inherit" }}>
              {label}
            </button>
          ))}
        </div>
      ...
      {/* Error */}
      {error && (
        <div style={{ background: `${C.danger}12`, border: `1px solid ${C.danger}44`, borderRadius: "10px", padding: "10px 14px", marginBottom: "14px", fontSize: "12.5px", color: C.danger, fontWeight: "600" }}>
          ⚠ {error}
        </div>
      )}
      ...
      {uploading && (
        <p style={{ textAlign: "center", fontSize: "11px", color: C.muted, marginTop: "8px" }}>
          ⏳ Uploading… this may take 30–120 seconds depending on file size. Please wait.
        </p>
      )}
