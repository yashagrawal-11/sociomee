...
function ResultPanel({ result, platform, keyword, isPro, onUpgradeClick, user }) {
  ...
  const downloadPDF = () => {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"/><style>body{font-family:Arial,sans-serif;max-width:800px;margin:0 auto;padding:40px;color:#1a0a00}.header{background:linear-gradient(135deg,#7c3aed,#ff3d8f);color:white;padding:24px 32px;border-radius:12px;margin-bottom:28px}.logo{font-size:28px;font-weight:900;margin-bottom:4px}.section{margin-bottom:24px;border-left:3px solid #7c3aed;padding-left:16px}.section-title{font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:#7c3aed;margin-bottom:8px}.title-box{background:#f5f3ff;border-radius:8px;padding:14px 16px;font-size:16px;font-weight:700;line-height:1.4}.script{font-size:13px;line-height:2;white-space:pre-wrap}.footer{margin-top:40px;text-align:center;font-size:11px;color:#8b6b9a;border-top:1px solid #e9e3ff;padding-top:16px}</style></head><body><div class="header"><div class="logo">SocioMee.</div><div style="font-size:12px;opacity:0.85">✦ AI Content Studio · ${plt} · ${date}</div></div>${title?`<div class="section"><div class="section-title">🎯 Best Title</div><div class="title-box">${title}</div></div>`:""}${hook?`<div class="section"><div class="section-title">🎣 Hook</div><p style="font-size:14px;font-weight:600;line-height:1.6">${hook}</p></div>`:""}${script?`<div class="section"><div class="section-title">📜 Full Script</div><div class="script">${script}</div></div>`:""}${desc?`<div class="section"><div class="section-title">📋 SEO Description</div><div class="script" style="font-size:12px">${desc}</div></div>`:""}${hashtags?`<div class="section"><div class="section-title">🏷️ Hashtags</div><p style="font-size:13px">${hashtags}</p></div>`:""}<div class="footer">Generated by SocioMee · sociomee.in</div></body></html>`;
