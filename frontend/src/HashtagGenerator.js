...
const PLATFORMS = [
  ...
  { id:"threads",   label:"Threads",   img:"/icons/threads.png"   },
  { id:"pinterest", label:"Pinterest", img:"/icons/pinterest.png" },
  { id:"facebook",  label:"Facebook",  img:"/icons/facebook.png"  },
  { id:"reddit",    label:"Reddit",    img:"/icons/reddit.png"    },
  { id:"telegram",  label:"Telegram",  img:"/icons/telegram.png"  },
  { id:"tiktok",    label:"TikTok",    img:"/icons/tiktok.png"    },
];
...
export default function HashtagGenerator({ user }) {
  ...
  return (
    <div style={{fontFamily:"'DM Sans','Syne',sans-serif",maxWidth:"800px",margin:"0 auto",padding:"40px 16px"}}>
      ...
      <div style={{marginBottom:"16px"}}>
        ...
        <div style={{display:"flex",gap:"8px",flexWrap:"nowrap"}}>
          <input value={keyword} onChange={e=>{setKeyword(e.target.value);setError("");}}
            ...
            onFocus={e=>{e.target.style.border="1.5px solid rgba(255,255,255,0.15)";e.target.style.boxShadow="0 none";}}
          ...
          <button onClick={generate} disabled={loading||!keyword.trim()}
            className="glow-btn"
            style={{padding:"11px 16px",borderRadius:"99px",border:`1.5px solid ${loading||!keyword.trim()?"rgba(255,255,255,0.08)":"rgba(255,255,255,0.25)"}`,background:loading||!keyword.trim()?"rgba(255,255,255,0.04)":"rgba(255,255,255,0.06)",backdropFilter:"blur(16px)",color:"#fff",fontWeight:"800",fontSize:"13px",cursor:loading||!keyword.trim()?"not-allowed":"pointer",fontFamily:"inherit",boxShadow:loading||!keyword.trim()?"none":"none",transition:"all 0.3s",whiteSpace:"nowrap",flexShrink:0}}>
            {loading?(
            ...
            {hashtags.map((tag,i)=>{
              ...
              return (
                <button key={i} onClick={()=>copyOne(tag)} title="Click to copy"
                  style={{padding:"8px 16px",borderRadius:"99px",border:`1.5px solid ${isCopied?"rgba(52,211,153,0.5)":"rgba(255,255,255,0.12)"}`,background:isCopied?"rgba(52,211,153,0.12)":"rgba(255,255,255,0.04)",color:isCopied?"#34d399":"rgba(255,255,255,0.6)",fontWeight:"600",fontSize:"13px",cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}
                  onMouseEnter={e=>{if(!isCopied){e.currentTarget.style.background="rgba(255,255,255,0.08)";e.currentTarget.style.boxShadow="none";}}}
