...
export function AuthCallback() {
  ...
  // Handle Microsoft/LinkedIn/Pinterest OAuth redirect
  useEffect(() => {
...
export default function LoginPage() {
  ...
  const sendOtp = async () => {
    if (!phone.trim()) { setPhoneErr("Enter your phone number"); return; }
    ...
    } catch(e) { setPhoneErr(e.message || "Failed to send OTP"); }
    setPhoneBusy(false);
  };

  const verifyOtp = async () => {
    if (!phoneOtp.trim()) { setPhoneErr("Enter OTP"); return; }
    setPhoneBusy(true); setPhoneErr("");
    try {
      const cred = await confirmResult.confirm(phoneOtp);
      const idToken = await cred.user.getIdToken();
      const r = await fetch(`${BASE}/auth/phone/verify`, {
        method: "POST",
        headers: {"Content-Type": "application/json"},
      ...
      await handleCallback(d.token);
      if (d.is_new) sessionStorage.setItem("show_onboarding", "true");
      window.location.href = d.is_new ? "/onboarding" : "/app";
    } catch(e) { setPhoneErr(e.message || "Invalid OTP"); }
    setPhoneBusy(false);
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#000000", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"24px", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      {phoneMode && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(16px)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
          <div style={{background:"rgba(10,8,20,0.85)",backdropFilter:"blur(24px)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"20px",padding:"28px",width:"100%",maxWidth:"400px",position:"relative"}}>
            <button onClick={()=>{setPhoneMode(false);setConfirmResult(null);setPhone("");setPhoneOtp("");setPhoneErr("");}} style={{position:"absolute",top:"16px",right:"16px",background:"none",border:"none",color:"rgba(255,255,255,0.35)",fontSize:"18px",cursor:"pointer"}}>✕</button>
            <div style={{fontSize:"15px",fontWeight:"800",color:"#fff",marginBottom:"4px"}}>Continue with Phone</div>
            <div style={{fontSize:"12px",color:"rgba(255,255,255,0.35)",marginBottom:"20px"}}>We will send an OTP to your number</div>
            {!confirmResult ? (
              <>
                <div style={{display:"flex",gap:"8px",marginBottom:"12px"}}>
                  <div style={{padding:"13px 14px",borderRadius:G.pill,border:G.border,background:G.bg,color:"rgba(255,255,255,0.5)",fontSize:"14px",whiteSpace:"nowrap",backdropFilter:G.blur}}>+91</div>
                  <input value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,""))} placeholder="10 digit mobile number" maxLength={10} style={{flex:1,padding:"13px 18px",borderRadius:G.pill,border:G.border,background:G.bg,color:"#fff",fontSize:"14px",fontFamily:"inherit",outline:"none",backdropFilter:G.blur}}
                    onFocus={e=>e.target.style.border="1px solid rgba(255,255,255,0.2)"}
                    onBlur={e=>e.target.style.border=G.border}
                  />
                </div>
                {phoneErr && <p style={{color:"rgba(239,68,68,0.8)",fontSize:"12px",margin:"0 0 10px"}}>{phoneErr}</p>}
                <button onClick={sendOtp} disabled={phoneBusy||phone.length<10} style={{width:"100%",padding:"14px",borderRadius:G.pill,border:G.border,background:G.bg,color:phone.length>=10?"#fff":"rgba(255,255,255,0.3)",fontWeight:"700",fontSize:"14px",cursor:phone.length>=10?"pointer":"default",fontFamily:"inherit",backdropFilter:G.blur}}>{phoneBusy?"Sending OTP...":"Send OTP"}</button>
              </>
            ) : (
              <>
                <div style={{fontSize:"12px",color:"rgba(255,255,255,0.35)",marginBottom:"12px"}}>OTP sent to +91{phone}</div>
                <input value={phoneOtp} onChange={e=>setPhoneOtp(e.target.value.replace(/\D/g,""))} placeholder="Enter 6 digit OTP" maxLength={6} style={{width:"100%",padding:"13px 18px",borderRadius:G.pill,border:G.border,background:G.bg,color:"#fff",fontSize:"14px",fontFamily:"inherit",outline:"none",backdropFilter:G.blur,marginBottom:"12px",letterSpacing:"4px"}}
                  onFocus={e=>e.target.style.border="1px solid rgba(255,255,255,0.2)"}
                  onBlur={e=>e.target.style.border=G.border}
                />
                {phoneErr && <p style={{color:"rgba(239,68,68,0.8)",fontSize:"12px",margin:"0 0 10px"}}>{phoneErr}</p>}
                <button onClick={verifyOtp} disabled={phoneBusy||phoneOtp.length<6} style={{width:"100%",padding:"14px",borderRadius:G.pill,border:G.border,background:G.bg,color:phoneOtp.length>=6?"#fff":"rgba(255,255,255,0.3)",fontWeight:"700",fontSize:"14px",cursor:phoneOtp.length>=6?"pointer":"default",fontFamily:"inherit",backdropFilter:G.blur}}>{phoneBusy?"Verifying...":"Verify OTP"}</button>
                <button onClick={()=>setConfirmResult(null)} style={{width:"100%",padding:"10px",borderRadius:G.pill,border:"none",background:"none",color:"rgba(255,255,255,0.3)",fontSize:"12px",cursor:"pointer",fontFamily:"inherit",marginTop:"8px"}}>Change number</button>
              </>
            )}
          </div>
        </div>
